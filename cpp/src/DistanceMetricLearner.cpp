...
class DistanceMetricLearner
{
	...
	public:
		...
		void Set_D_ij_Constraint_map(IloEnv env, IloModel model, IloNumVarArray a, NumVarMatrix B, IloNumVarArray D_array, int num, int i, int j)
		{
				...
				model.add(D_array[num] == D_ij_temp);
				D_ij_temp.end();		
		...
		void learn_metric_LP_fast_real()
		{
			IloEnv env;
			try {
				//definition of variables
					...
					for (int j = 0; j < n; j++)
					{
						if (i != j)
						{
							...
							if (V1_0[i][j] == 1)
							{
								...
								//model.add(D[i][j] == D_ij_temp);
								model.add(t[i] + epsilon <= D_ij_temp);
								D_ij_temp.end();
						...
						if (outliers[i] == 0)
						{
							...
							if (temp > threshold)
							{
								...
								for (int k = 0; k < d; k++)
								{
									if (include_first_order_terms)
									{
										D_ij_temp += fabs(features[ii][k] - features[jj][k]) * a[k];
									}

									D_ij_temp += (features[ii][k] - features[jj][k]) * (features[ii][k] - features[jj][k]) * B[k][0];
								}
								for (int p = 0; p < d - 1; p++)
								{
									for (int l = p + 1; l < d; l++)
									{
										D_ij_temp += 2 * (features[ii][p] - features[jj][p])* (features[ii][l] - features[jj][l]) * B[p][l - p];
									}
								}
								//model.add(D[ii][jj] == D_ij_temp);
								model.add(t[i] <= D_ij_temp);
						...
						if (outliers[i] == 0)
						{
							...
							else
							{
								...
								for (int k = 0; k < d; k++)
								{
									if (include_first_order_terms)
									{
										D_ij_temp += fabs(features[ii][k] - features[kk][k]) * a[k];
									}

									D_ij_temp += (features[ii][k] - features[kk][k]) * (features[ii][k] - features[kk][k]) * B[k][0];
								}
								for (int p = 0; p < d - 1; p++)
								{
									for (int l = p + 1; l < d; l++)
									{
										D_ij_temp += 2 * (features[ii][p] - features[kk][p])* (features[ii][l] - features[kk][l]) * B[p][l - p];
									}
								}
								//model.add(D[ii][kk] == D_ij_temp);
								model.add(t[ii] + epsilon <= D_ij_temp);
		...
		void learn_metric_LP_fast_test(bool erase_map)
		{
			IloEnv env;
			try {
				//definition of variables
...
int main()
{
	...
	//_model.learn_metric_LP_fast_test(1);
